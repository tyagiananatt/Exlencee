...
// ----------------------------------------------------------------------
// FIX: Change 'db_connection.php' to the fully configured 'config.php'
// ----------------------------------------------------------------------
require_once 'config.php'; 
// NOTE: $conn variable is available if config.php *only* contains the connection setup.
// If config.php is the version with the connectDB() function, you need to use that function.

// Since the old db_connection.php returned $conn directly, we'll assume 
// your config.php is the simple connection file for now, or ensure $conn is set.
// If you are using the function-based config.php, you must explicitly call it:
$conn = connectDB();
if (!$conn) {
    // If connection fails, stop script immediately
    die("Connection failed to establish in login.php.");
}
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                if ($login_stmt = $conn->prepare($login_sql)) {
                    ...
                    $login_stmt->close(); // Close the login statement
                }
                
                // Close the main statement and connection before redirect
                $stmt->close();
                $conn->close();
