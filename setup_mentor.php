...
$normalize_sql = "UPDATE users SET username = 'mentor' WHERE LOWER(TRIM(username)) = 'mentor'";
...
if ($result->num_rows == 0) {
    ...
    // Update existing mentor account: ensure role and reset password to default
    $new_password = password_hash("mentor123", PASSWORD_DEFAULT);
    $update_sql = "UPDATE users SET role = 'mentor', password = ? WHERE username = 'mentor'";
    $stmt = $conn->prepare($update_sql);
    if ($stmt) {
        $stmt->bind_param("s", $new_password);
        if ($stmt->execute()) {
            echo "Mentor account updated successfully.<br>";
            echo "Username: mentor<br>";
            echo "Password: mentor123<br>";
        } else {
            echo "Error updating mentor account: " . $conn->error;
        }
        $stmt->close();
    } else {
...
// Verify mentor credentials now
$verify_sql = "SELECT id, username, role, password FROM users WHERE username = 'mentor'";
$verify_res = $conn->query($verify_sql);
if ($verify_res && $verify_res->num_rows === 1) {
    $u = $verify_res->fetch_assoc();
    $ok = password_verify('mentor123', $u['password']);
    echo $ok ? "Mentor password set OK.<br>" : "Mentor password verification failed.<br>";
    echo "Role: " . htmlspecialchars($u['role']) . "<br>";
}
